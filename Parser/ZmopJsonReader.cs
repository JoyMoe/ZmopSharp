...
namespace Zmop.Api.Parser
{
    ...
    /// ZMOP JSON响应通用读取器。
    /// </summary>
    public class ZmopJsonReader : IZmopReader
    {
        private IDictionary json;

        public ZmopJsonReader(IDictionary json)
        {
            this.json = json;
        }
        ...
        public object GetReferenceObject(object name, Type type, DZmopConvert convert)
        {
            IDictionary dict = json[name] as IDictionary;
            if (dict != null && dict.Count > 0)
            {
                return convert(new ZmopJsonReader(dict), type);
            }
            else
            {
                return null;
        ...
        public IList GetListObjects(string listName, string itemName, Type type, DZmopConvert convert)
        {
            IList listObjs = null;
            ...
            if (jsonList != null && jsonList.Count > 0)
            {
                ...
                foreach (object item in jsonList)
                {
                    if (typeof(IDictionary).IsAssignableFrom(item.GetType())) // object
                    {
                        ...
                        object subObj = convert(new ZmopJsonReader(subMap), type);
                        if (subObj != null)
                        {
                            listObjs.Add(subObj);
                        }
